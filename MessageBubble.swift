...
//  SocialMedia
//
//  Created by Shaurya Baranwal on 4/20/22.
//
...
struct MessageBubble: View {
    ...
    var body: some View {
        VStack(alignment: message.received ? .leading : .trailing){
            HStack{
                Text(message.text).padding().background(message.received ? Color("Gray") : Color("Red")).cornerRadius(30)
...
struct MessageBubble_Previews: PreviewProvider {
    static var previews: some View {
        MessageBubble(message: Message(id: "12345", text: "I have been coding SwiftUI", received: true, timestamp: Date()))
    }
}
